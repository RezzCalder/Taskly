...
@Composable
fun SelesaiPribadiScreen(navController: NavController) {
    ...
    LaunchedEffect(Unit) {
        ...
        api.getPersonalTasksCompleted(userId).enqueue(object : Callback<List<PersonalTask>> {
            override fun onResponse(call: Call<List<PersonalTask>>, response: Response<List<PersonalTask>>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(context, "Gagal memuat tugas selesai.", Toast.LENGTH_SHORT).show()
                }
            }
