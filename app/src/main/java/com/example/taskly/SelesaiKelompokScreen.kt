...
@Composable
fun SelesaiKelompokScreen(navController: NavController) {
    ...
    LaunchedEffect(Unit) {
        ...
        api.getGroupTasksCompleted(userId).enqueue(object : Callback<List<GroupTask>> {
            override fun onResponse(call: Call<List<GroupTask>>, response: Response<List<GroupTask>>) {
                if (response.isSuccessful) tasks.addAll(response.body() ?: emptyList())
            }
